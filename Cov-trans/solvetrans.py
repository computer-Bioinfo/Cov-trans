...
class MILP_solve():
    ...
    def solve_milp(self,edges,Path_edge2,Path_edge1,Path_node1,Path_node2,all_calpaths,all_nonpaths,Path,Node_edge,node1_flow,edge_flow,outfile):
        ...
        #obj
        w1= 1/len(all_calpaths+all_nonpaths)
        w2= len(all_nonpaths+all_calpaths)/(len(self.graph.nodes)+1)
        w3= len(all_nonpaths+all_calpaths)/len(edges)
        # print(len(all_nonpaths+all_calpaths))
        # print(len(self.graph.nodes)+1)
        # print(len(edges))
        # print(w1)
        # print(w2)
        # print(w3)
    ...
    def writecalgtf(self, filename,transcripts_all, contig='NC_045512.2',type='cal'):
        ...
        print("Canonical transicripts：GTF file written successfully!")
    ...
    def writenongtf(self, filename, transcripts_all, results, contig='NC_045512.2', type='non'):
        ...
        with open(filename, 'w') as output:
            def write_transcript_and_exons(contig, path):
                ...
                output.write(f"{contig}\tCov-rans\ttranscript\t{path[0][0]+1}\t{path[-1][1]+1}\t.\t.\t.\t\n")
                ...
                output.write(f"{contig}\tCov-rans\texon\t{exon_start+1}\t{path[-1][1]+1}\t.\t.\t.\t\n")
            ...
            for transcript_info, result in zip(transcripts_all, results):
                if result == 1:
                    ...
                    # print("Path:", path)
        print("Noncanonical-transicripts：GTF file written successfully!")
