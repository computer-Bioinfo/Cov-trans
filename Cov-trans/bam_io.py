...
class SpanningReads():
    ...
    def merge_adjacent_keys(non_trans_more, tolerance):
        from collections import defaultdict
        import numpy as np

        # Helper function to round p5 values
        def round_p5(p5, tolerance):
            return p5 // tolerance * tolerance

        # Step 1: Round p5 values and accumulate counts in a list
        rounded_non_trans_more = defaultdict(list)
        for (p5, p3), value in non_trans_more.items():
            rounded_p5 = round_p5(p5, tolerance)
            rounded_key = (rounded_p5, p3)
            rounded_non_trans_more[rounded_key].append((p5, value['non']))

        # Step 2: Compute the median of p5 values and sum the corresponding counts
        final_non_trans_more = {}
        for (rounded_p5, p3), values in rounded_non_trans_more.items():
            p5_values = [p5 for p5, _ in values]
            non_values = [non for _, non in values]

            # Calculate the median of p5 and sum the corresponding counts
            median_p5 = int(np.median(p5_values))
            total_non = sum(non_values)

            # Use the median p5 as the final key and sum of counts as the value
            final_key = (median_p5, p3)
            final_non_trans_more[final_key] = {'non': total_non}

        return final_non_trans_more
    ...
    def POScoverage(self,bam_file):
        bam = pysam.AlignmentFile(bam_file, "rb")
        coverage_list = []

        # 遍历 BAM 文件中的每个位置
        for pileupcolumn in bam.pileup():
            pos = pileupcolumn.pos # 将0基转换为1基
            cov = pileupcolumn.nsegments

            coverage_list.append(cov)

        bam.close()
        # print("len",len(coverage_list))
        # print(coverage_list)
    ...
    def get_spanning_reads(self, bam_file,poscoverage, mutList=[], paired=False) -> pd.DataFrame:
        bam = pysam.AlignmentFile(bam_file, "rb")
        ...
        for _, row in grouped_df.iterrows():
            ...
            if row["5prime"] >= 50 and row["5prime"] <= 85:
                ...
                un_end = start + 3 + min(self.reference.fetch(self.contig, start + 3, self.refLength).find("TAA"),
                                         ...
                                         self.reference.fetch(self.contig, start + 3, self.refLength).find(
                                             "TGA"))  # 从起始密码子后一个开始找
        ...
        unend_orf1a = start_orf1ab + 3 + min(
            self.reference.fetch(self.contig, start_orf1ab + 3, constraint_1ab).find("TAA"),
        ...
        non_trans_after = {k: v for k, v in non_trans.items() if v.get('non', 0) > reads_count*0.005}
        ...
        return grouped_df, base_ref, can_trans, non_trans_splice, orf1ab_trans
